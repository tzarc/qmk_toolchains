...
RUN mkdir -p /qmk/bin \
    && curl -fsSLO https://github.com/indygreg/apple-platform-rs/releases/download/apple-codesign%2F${RCODESIGN_VERSION}/apple-codesign-${RCODESIGN_VERSION}-x86_64-unknown-linux-musl.tar.gz \
...
# Toolchain tarballs change every run -- keep this after the tool builds so their layers stay cacheable
COPY qmk_toolchain*.tar.zst /tmp/
RUN ls -1 /tmp/qmk_toolchain*.tar.zst \
